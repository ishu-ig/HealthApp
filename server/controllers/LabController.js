...
async function createRecord(req, res) {
    try {
        let data = new Lab(req.body)
        if (req.file) {
            data.pic = req.file.path
        }
        data.availableDays = [].concat(req.body.availableDays)
        await data.save()
        res.send({
            result: "Done",
            data: data
        ...
        let errorMessage = {}
        error.keyValue ? errorMessage.name = "Lab Already Exist" : null,
        ...
        error.errors?.openingTime ? errorMessage.openingTime = error.errors.openingTime.message : null;
...
async function getRecord(req, res) {
    try {
        let data = await Lab.find().sort({ _id: -1 })
        res.send({
            result: "Done",
            count: data.length,
            data: data
...
async function getSingleRecord(req, res) {
    try {
        let data = await Lab.findOne({ _id: req.params._id })
        if (data)
            res.send({
                result: "Done",
                data: data
...
async function updaterecord(req, res) {
    try {
        ...
        if (data) {
            ...
            data.openingTime = req.body.openingTime ?? data.openingTime,
