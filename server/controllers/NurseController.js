...
async function createRecord(req, res) {
    try {
        let data = new Nurse(req.body)
        if (req.file) {
            data.pic = req.file.path
        }
        await data.save()
        let finalData = await Nurse.findOne({ _id: data._id })
...
async function getRecord(req, res) {
    try {
        let data = await Nurse.find().sort({ _id: -1 })
            .populate("hospital", ["name"])
        res.send({
            result: "Done",
            count: data.length,
...
async function getSingleRecord(req, res) {
    try {
        let data = await Nurse.findOne({ _id: req.params._id })
            .populate("hospital", ["name"])
        if (data)
            res.send({
                result: "Done",
    ...
    try {
        ...
        if (data) {
            data.name = req.body.name ?? data.name,
                data.hospital = req.body.hospital ?? data.hospital,
