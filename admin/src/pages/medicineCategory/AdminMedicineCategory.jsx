...
import { deleteMedicineCategory, getMedicineCategory } from "../../Redux/ActionCreators/MedicineCategoryActionCreators";

export default function AdminMedicineCategory() {
    ...
    function deleteRecord(_id) {
        if (window.confirm("Are you sure you want to delete this item?")) {
            dispatch(deleteMedicineCategory({ _id: _id }));
            getAPIData();
            setFlag(!flag)
        }
    }
    ...
    return (
        <>
            <div className="container-fluid">
                {/* Header */}
                <h5 className="text-center text-light bg-primary p-3">MedicineCategory <Link to="/MedicineCategory/create"><i className="fa fa-plus text-light float-end pt-1"></i></Link></h5>
                {/* Table */}
                <div className="table-responsive mt-3">
                    <table id="DataTable" className="table table-striped table-hover table-bordered text-center">
                        ...
                        <tbody>
                            {MedicineCategoryStateData.map((item) => (
                                <tr key={item._id}>
                                    <td>{item._id}</td>
                                    <td>{item.name}</td>
                                    <td>
                                    ...
                                    <td>
                                        <Link to={`/medicineCategory/update/${item._id}`} className="btn btn-primary text-light btn-sm">
