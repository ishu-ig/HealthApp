...
import {
  getMedicineCheckout,
  updateMedicineCheckout,
} from '../../Redux/ActionCreators/MedicineCheckoutActionCreators';

export default function AdminMedicineCheckoutShow() {
  const { _id } = useParams();
  ...
  /* Fetch delivery boys from user list */
  useEffect(() => {
  ...
  useEffect(() => {
    const list = Array.isArray(MedicineCheckoutStateData)
      ? MedicineCheckoutStateData
      : MedicineCheckoutStateData?.data || [];
    if (list.length) {
  ...
  function updateRecord() {
    ...
    if (window.confirm('Are you sure you want to update the status?')) {
      const updated = { ...data, orderStatus, paymentStatus, deliveryBoy: selectedDeliveryBoy || undefined };
      dispatch(updateMedicineCheckout(updated));
      setData(updated);
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
      navigate('/admin/medicine-checkout');
    }
  }
  ...
  /* Status steps — matches MedicineCheckout orderStatus lifecycle */
  const STATUS_STEPS = [
    ...
    <div className="acs-root fade-in-up">
      ...
      <div className="acs-page-header">
        <div className="acs-page-header-left">
          <Link to="/admin/medicine-checkout" className="acs-back-btn">
            <i className="fas fa-arrow-left"></i>
          </Link>
          <div>
            <h5 className="acs-page-title">
              <i className="fas fa-pills me-2" style={{ color: 'var(--accent)' }}></i>Medicine Order Details
            </h5>
            <p className="acs-page-id">ID: {data._id}</p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <span className={`acs-badge ${isCancelled ? 'danger' : data.orderStatus === 'Delivered' ? 'success' : data.orderStatus === 'Out for Delivery' ? 'teal' : 'info'}`}>
      ...
      <div className="acs-grid">
        ...
        {/* ── Order Summary — uses MedicineCheckout schema fields ── */}
        <div className="acs-card">
          <div className="acs-card-head">
          ...
          <div className="acs-summary-grid">
            ...
            {/* MedicineCheckout schema: field is "shipping" */}
            <div className="acs-summary-row">
              <span>Shipping</span>
            ...
            {/* MedicineCheckout schema: rppid (Razorpay payment id) */}
            {data.rppid && (
              <div className="acs-summary-row">
            ...
            {/* MedicineCheckout schema: deliveryBoy (ObjectId ref) */}
            {data.deliveryBoy && (
              <div className="acs-summary-row">
                <span>Delivery Boy</span>
                <span>{data.deliveryBoy?.name || data.deliveryBoy}</span>
              </div>
            )}
          </div>
        </div>

        {/* ── Medicines — MedicineCheckout schema: medicines[] ── */}
        {data.medicines?.length > 0 && (
          <div className="acs-card acs-full">
            <div className="acs-card-head">
              <span><i className="fas fa-pills me-2" style={{ color: 'var(--accent)' }}></i>Ordered Medicines</span>
              <span className="acs-count-pill">{data.medicines.length} item{data.medicines.length > 1 ? 's' : ''}</span>
            </div>
                  ...
                  {data.medicines.map((m, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>
                        {m.name || m.medicine?.name || '—'}
                      </td>
                      <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                        {m.medicineCategory || '—'}
                      </td>
                      <td>
                        <span style={{ background: 'var(--accent-glow)', color: 'var(--accent)', padding: '2px 9px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                          ×{m.qty ?? 1}
                        </span>
                      </td>
                      <td>₹{m.price ?? 0}</td>
                      <td style={{ fontWeight: 700, color: 'var(--accent)' }}>₹{m.total ?? 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {isCompleted && (
          <div className="acs-card acs-full acs-completed-banner">
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#2d7a50', marginTop: 2 }}>Delivered and payment received.</div>
            </div>
          </div>
        )}
...
const STYLES = `
  ...
  .acs-update-grid { display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:20px 20px 0; }
