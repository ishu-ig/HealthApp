...
export default function AdminCreateHospital() {
  ...
  function postSubmit(e) {
    ...
    if (errorItem) setShow(true);
      ...
      // let item = HospitalStateData.find(x => x._id !== id && x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase())
      if (item) {
        setShow(true);
        setError((old) => {
          return {
            ...old,
            name: "Hospital Already Exist",
          };
        });
      } else {
  ...
  if (HospitalStateData.length) {
    let item = HospitalStateData.find((x) => x._id === _id);
        ...
        <form onSubmit={postSubmit}>
          ...
          <div className="mb-3 pt-3">
            <button type="submit" className="btn btn-primary w-100 text-light">
              <i className="fa fa-save"></i> Update Hospital
            </button>
          </div>
        </form>
      </div>
