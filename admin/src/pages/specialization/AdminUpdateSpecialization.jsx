...
import { updateSpecialization, getSpecialization } from '../../Redux/ActionCreators/SpecializationActionCreators'

export default function AdminUpdateSpecialization() {
    // let { id } = useParams()
    ...
    function postSubmit(e) {
        ...
        if (errorItem)
            ...
            let item = SpecializationStateData.find(x => x._id !== _id && x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase())  // in case of real backend
            // let item = SpecializationStateData.find(x => x._id !== id && x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase())
            if (item) {
                setShow(true)
                setError((old) => {
                    return {
                ...
                // dispatch(updateSpecialization({ ...data }))
                ...
                dispatch(updateSpecialization(formData))
    ...
    useEffect(() => {
        (() => {
            ...
            if (SpecializationStateData.length) {
                // let item = SpecializationStateData.find(x => x._id === id)
                let item = SpecializationStateData.find(x => x._id === _id)
                if (item)
                    setData({ ...item })
            }
        })()
    ...
    return (
        <>
            <div className="container">
                <h5 className="text-center text-light bg-primary p-2">Update Specialization <Link to="/specialization"><i className="fa fa-arrow-left text-light float-end pt-1"></i></Link></h5>
                {/* Form */}
