...
import { deleteSpecialization, getSpecialization } from "../../Redux/ActionCreators/SpecializationActionCreators";
...
export default function AdminSpecialization() {
    ...
    function deleteRecord(_id) {
        if (window.confirm("Are you sure you want to delete this item?")) {
            dispatch(deleteSpecialization({ _id: _id }));
            getAPIData();
            setFlag(!flag)
        }
    }
    ...
    return (
        <>
            <div className="container-fluid">
                {/* Header */}
                <h5 className="text-center text-light bg-primary p-3">Specialization <Link to="/Specialization/create"><i className="fa fa-plus text-light float-end pt-1"></i></Link></h5>
                {/* Table */}
                <div className="table-responsive mt-3">
                    <table id="DataTable" className="table table-striped table-hover table-bordered text-center">
                        ...
                        <tbody>
                            {SpecializationStateData.map((item) => (
                                <tr key={item._id}>
                                    <td>{item._id}</td>
                                    <td>{item.name}</td>
                                    <td>
                                    ...
                                    <td>
                                        <Link to={`/Specialization/update/${item._id}`} className="btn btn-primary text-light btn-sm">
