...
var rte;
export default function AdminUpdateNurse() {
  let { _id } = useParams();
  ...
  function postSubmit(e) {
    ...
    if (errorItem) {
      ...
      if (item) {
        setShow(true);
        setError((old) => ({ ...old, name: "Nurse Already Exist" }));
      } else {
  ...
  // ✅ single useEffect — fetch Nurses and prefill form
  useEffect(() => {
    (() => {
      dispatch(getNurse());
        ...
        <form onSubmit={postSubmit}>
          ...
          <div className="mb-3 pt-3">
            <button type="submit" className="btn btn-primary w-100 text-light">
              <i className="fa fa-save"></i> Update Nurse
            </button>
          </div>
        </form>
      </div>
